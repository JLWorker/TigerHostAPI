...
public class TwoFactorFacade {
    ...
    public Mono<QrCodeData> getQrCode(){
        return mainFacadeUtils.getPrincipal()
                ...
                .switchIfEmpty(mainFacadeUtils.getNotFoundException("Secret code not found for user, turn on two factor status"))
    ...
    public Mono<TokensResponse> verifyCode(TwoFactorCode twoFactorCode, String token, String ipAddress){
        return tokenService.get2FaTokenData(token)
                .flatMap(deviceToken -> userRepository.getUserByTwoFactorDeviceToken(deviceToken)
                    ...
                    .flatMap(user -> twoFactorService.verify(user.getTwoFactorSecret(), twoFactorCode.getCode())
                        .filter(res -> res)
                        ...
                        .then(mainFacadeUtils.generatePairTokens(user, twoFactorCode.getDeviceData(), ipAddress)
                                .flatMap(tokensResponse -> kafkaMessageFactory.createKafkaMessage(KafkaMessageType.FEEDBACK_MESSAGE, FeedbackEventType.UPDATE_TOKENS.getName(), null)
                                        .flatMap(kafkaMessage -> mainFacadeUtils.sendMessageInKafkaTopic(kafkaMessage, feedbackTopic, new FeedbackHeadersDTO(user.getUserCode()), PartitioningStrategy.BASIC_MESSAGES))
