...
public class RestControllerAdvice {
    ...
    public ResponseEntity<ExceptionResponse> runtimeHandlerException(RuntimeException exception, ServerWebExchange exchange) {
        ...
        if (exception instanceof TwoFactorActiveException) {
            ...
            headers.add("Logout", "true");
        }
        ExceptionResponse response = new ExceptionResponse(exchange.getRequest().getPath().value(), HttpStatus.UNAUTHORIZED.getReasonPhrase(), HttpStatus.UNAUTHORIZED.value(), exception.getMessage());
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).headers(httpHeaders -> httpHeaders.addAll(headers)).body(response);
